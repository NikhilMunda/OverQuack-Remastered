...
def executeNonRecursive(line, remaining_lines):
    ...
    """Non-recursive execution function to prevent stack exhaustion"""
    ...
    try:
        ...
        result = parseLine(line, SafeIterator(remaining_lines))

        return list(result) if hasattr(result, '__iter__') else remaining_lines
...
def parseLine(line, script_lines):
    ...
    """Main code handler - FIXED to prevent stack exhaustion and handle IF blocks properly"""
    ...
    if line.startswith("JIGGLE_MOUSE"):
        ...
        print(f'[SCRIPT_LINES_REM]: {BetterListOutput(remaining)}\n')

        loopCode = _getCodeBlock(SafeIterator(remaining))
        ...
        while evaluateExpression(replaceAll(condition)) == True and iteration_count < max_iterations:
            ...
            # Execute loop body without recursion

            for loop_line in loopCode:

                executeNonRecursive(loop_line, [])
        ...
        # Skip past the loop code

        depth = 1

        skip_count = 0

        for skip_line in remaining:

            if skip_line.strip().upper().startswith("END_WHILE"):

                depth -= 1

            elif skip_line.strip().upper().startswith("WHILE"):
        ...
        # Collect all IF/ELSE_IF/ELSE blocks till END_IF
        remaining = list(script_lines.remaining()) if hasattr(script_lines, 'remaining') else list(script_lines)
        blocks, lines_consumed = extract_conditional_blocks(iter(remaining))
        ...
        if result:
            ...
            for block_type, block_content in blocks:
                if block_type == 'IF':
                    chosen = block_content
                    break
        else:
            # IF condition is false, check ELSE_IF conditions then ELSE
            for block_type, block_content in blocks:
                if block_type == 'ELSE_IF_TRUE':
                    chosen = block_content
                    break
        ...
        if chosen:
            print_with_color(f"[IF_EXECUTION] Executing {len(chosen)} lines from chosen branch", BRIGHT_GREEN)
            for block_line in chosen:
                if block_line.strip():
                    parseLine(block_line, SafeIterator([]))
    ...
    # FUNCTION EXECUTION
        ...
        # Execute function lines without recursion

        for func_line in functions[line]:

            executeNonRecursive(func_line, [])
...
def runScript(duckyScriptPath):
                    ...
                    if line.startswith("SELECT_LAYOUT"):
                        ...
                        for _ in range(times_value):
                            ...
                            for repeated_line in previousLines[-lines_value:]:
                                ...
                                executeNonRecursive(repeated_line, [])
    ...
    print(f"[{duckyScriptPath.upper().replace('.oqs', '')}_COMPLETED] 200")
